...
<head>
  ...
  <style>
    body {{ background: #eef2ff; color: #0f172a; font-family: system-ui, sans-serif; margin: 0; line-height: 1.45; }}
    main {{ display: grid; gap: 1rem; margin: 0 auto; max-width: 72rem; padding: clamp(1rem, 4vw, 3rem); }}
    .hero {{ background: linear-gradient(135deg, #172554, #2563eb); border-radius: 1.2rem; color: white; padding: clamp(1.5rem, 4vw, 3rem); }}
    .brand-row {{ align-items: center; display: flex; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.5rem; }}
    .brand-logo {{ background: white; border-radius: 0.8rem; box-shadow: 0 12px 30px rgb(15 23 42 / 0.18); display: block; height: 4.5rem; object-fit: contain; padding: 0.55rem; width: auto; }}
    .brand-logo-carlab {{ height: 5.5rem; }}
    .eyebrow {{ margin: 0; opacity: 0.85; text-transform: uppercase; letter-spacing: 0.14em; font-weight: 800; }}
    .panel {{ background: white; border: 1px solid #cbd5e1; border-radius: 1rem; box-shadow: 0 12px 32px rgb(15 23 42 / 0.08); padding: 1rem; }}
    ...
    @media (max-width: 520px) {{ .brand-logo {{ height: 3.4rem; max-width: 100%; }} .brand-logo-carlab {{ height: 4.2rem; }} }}
  </style>
</head>
<body>
  <main>
    <header class="hero">
      <div class="brand-row" aria-label="University of Delaware and CAR Lab logos">
        <img class="brand-logo" src="../logo/UDLogo.jpg" alt="University of Delaware logo">
        <img class="brand-logo brand-logo-carlab" src="../logo/CARLabLogo.png" alt="CAR Lab logo">
      </div>
      <p class="eyebrow">ADA Prelearning</p>
